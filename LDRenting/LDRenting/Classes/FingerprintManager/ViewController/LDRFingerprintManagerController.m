//
//  LDRFingerprintManagerController.m
//  LDRenting
//
...
@interface LDRFingerprintManagerController ()
@property (nonatomic, strong) UIView *bottomView;
@property (nonatomic, strong) NSArray *dataSource;
@end

@implementation LDRFingerprintManagerController
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    [label setText:@"  tips：每个用户最多只能添加两个指纹  "];
...
- (UIView *)bottomView
{
    if (!_bottomView) {
        ...
        [button setTitle:@"添加指纹" forState:UIControlStateNormal];
...
{
    if (!_dataSource) {
        _dataSource = @[@"指纹开门权限",@"大菠萝的指纹1",@"大菠萝的指纹2",@"大菠萝的指纹3"];
    }
    return _dataSource;
}
