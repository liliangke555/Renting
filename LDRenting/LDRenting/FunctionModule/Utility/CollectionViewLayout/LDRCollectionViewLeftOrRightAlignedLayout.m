...
@interface LDRCollectionViewLeftOrRightAlignedLayout ()
...
@implementation LDRCollectionViewLeftOrRightAlignedLayout
...
- (void)prepareLayout {
    ...
    self.viewWidth = LDR_WIDTH - self.itemInset.left - self.itemInset.right;
     
//    NSLog(@"-----------------:%f  %f  %f",LDR_WIDTH ,self.itemInset.left ,self.itemInset.right);
    //所有内容的布局属性数组
...
- (void)setItemFrameWithIndexPath:(NSIndexPath *)indexPath {
    ...
    CGFloat x = 0.0;
    CGFloat y = 0.0;
    CGFloat width = [self countItemSizeWidthWithIndexPath:indexPath];
    CGFloat height = [self countItemSizeWithIndexPath:indexPath];
    ...
    /** 添加frame */
    if(width >= self.viewWidth-self.interitemSpacing*2){
        attributes.frame = CGRectMake(x, y, self.viewWidth-self.interitemSpacing*2, height);
    }else{
        attributes.frame = CGRectMake(x, y, width, self.itemHeight);
    }
...
- (void)makeHeaderAttributesWithIndexPath:(NSIndexPath *)indexPath lastItemAttributes:(UICollectionViewLayoutAttributes *)attributes {
    ...
    if ( [self.delegate respondsToSelector:@selector(collectionViewDynamicHeaderSizeWithIndexPath:)] ) {
        ...
        headerWidth = LDR_WIDTH - self.headerInset.left - self.headerInset.right;
...
- (void)makeFooterAttributesWithLastItemAttributes:(UICollectionViewLayoutAttributes *)attributes {
    ...
    if ( [self.delegate respondsToSelector:@selector(collectionViewDynamicFooterSizeWithIndexPath:)] ) {
        ...
        footerWidth = LDR_WIDTH - self.footerInset.left - self.footerInset.right;
...
- (CGFloat)countItemSizeWithIndexPath:(NSIndexPath *)indexPath {
    ...
    CGSize size =   [content boundingRectWithSize:CGSizeMake(self.viewWidth-self.interitemSpacing*2, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:self.labelFont} context: nil].size;
    return MAX(size.height, self.itemHeight);
}
- (CGFloat)countItemSizeWidthWithIndexPath:(NSIndexPath *)indexPath{
    NSString *content = [self.delegate collectionViewItemSizeWithIndexPath:indexPath];
    CGSize size =   [content boundingRectWithSize:CGSizeMake(MAXFLOAT, 10) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:self.labelFont} context: nil].size;
//    NSLog(@"==========================:%@",self.labelFont);
