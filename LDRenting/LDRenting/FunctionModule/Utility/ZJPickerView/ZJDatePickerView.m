...
- (instancetype)initWithTitle:(NSString *)title
    ...
    if (self = [super init]) {
        ...
        if (defaultSelValue && defaultSelValue.length > 0) {
            ...
            if (!defaultSelDate) {
//                XZLog(@"参数格式错误！参数 defaultSelValue 的正确格式是：%@", self.selectDateFormatter);
...
#ifdef DEBUG
//        XZLog(@"最小时间date：%@", self.minLimitDate);
//        XZLog(@"默认时间date：%@", self.selectDate);
//        XZLog(@"最大时间date：%@", self.maxLimitDate);
//        
//        XZLog(@"最小时间：%@", [NSDate zj_getDateString:self.minLimitDate format:self.selectDateFormatter]);
//        XZLog(@"默认时间：%@", [NSDate zj_getDateString:self.selectDate format:self.selectDateFormatter]);
//        XZLog(@"最大时间：%@", [NSDate zj_getDateString:self.maxLimitDate format:self.selectDateFormatter]);
#endif
...
-(void)showPickerViewWithAnimation:(BOOL)animation{
    ...
    if (animation) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            CGRect rect = self.alertView.frame;
            rect.origin.y -= kZJPickerHeight + kZJTopViewHeight;
            self.alertView.frame = rect;
        }];
    }
}
